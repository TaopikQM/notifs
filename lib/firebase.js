...
import { initializeApp, getApps, getApp } from 'firebase/app';
import { getDatabase, ref, set, onValue, push, update, get, remove, off,  onDisconnect, serverTimestamp, } from 'firebase/database';
...
export { getToken, onMessage };
 ...
 export { rtdb,db,database, storage, push, update, get, set, ref, off, onValue, remove, onDisconnect, serverTimestamp, storageBackup,storageBackup1};//,db, storage,   auth, signInWithEmailAndPassword, signInWithPopup,  createUserWithEmailAndPassword, GoogleAuthProvider
...
// Helper: Format last seen menjadi lebih singkat
...
// Helper: Set user online
export const setUserOnline1 = async (userId) => {
  ...
  await set(presenceRef, {
    ...
    connectedAt: wib.full,
  });
};

// Helper: Set user offline dengan last seen
export const setUserOffline1 = async (userId) => {
  const wib = getWIBTime();
  const presenceRef = ref(database, `user-presence/${userId}`);
...
// Helper: Get user presence
export const getUserPresence1 = async (userId) => {
  try {
    const snapshot = await get(ref(database, `user-presence/${userId}`));
...
// Helper: Set user online dengan heartbeat
...
// Helper: Heartbeat - update lastSeen setiap interval
export const startHeartbeat = (userId, interval = 30000) => {
  const wib = getWIBTime();
  const presenceRef = ref(database, `user-presence/${userId}`);
  ...
  const heartbeat = setInterval(async () => {
    try {
      ...
      console.log(`[Heartbeat] Updated for ${userId}`);
    } catch (err) {
      console.error("Heartbeat error:", err);
    }
  }, interval);

  return heartbeat; // Return untuk cleanup
};
...
export const getUserPresence = async (userId) => {
  ...
  const sortedUsers = [...users].sort((a, b) => {
    // Ambil nilai createdAt dengan fallback jika undefined
